 ...
 *
 * Что шлёт:
 *  - точный тех.статус (Крутит N/M, Отклонены+причина, Бюджет, Пауза/Оплата)
 *  - почасовой спенд за сегодня+вчера по поясу аккаунта + сам пояс
 *  - детализацию по кампаниям (для раскрытия в карточке)
 *  - нативную валюту
 * «Пауза/Оплата» = на практике дефолтная ветка означает overdue balance
 * (Google паузит кампании из-за неоплаты). Окончательное распознавание
 * payment-токена сделаем после диагностики на живых аккаунтах.
  ...
  try {
    ...
    var payload = {
      ...
      batch: [{
        ...
        tech_status: status.status,   // Крутит | Отклонены | Бюджет | Пауза/Оплата
...
function computeStatus(campaigns) {
  ...
  // Дефолтная ветка: на практике почти всегда — overdue balance.
  // Помечаем как «Пауза/Оплата», подсвечивается оранжевым в UI.
  return { status: 'Пауза/Оплата', note: '' };
}
