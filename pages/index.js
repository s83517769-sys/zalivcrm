...
// литерал, что и при настоящей паузе/оплате (отдельного статуса moderation
// нет). Различаем по возрасту, плюс отдельно отрабатываем кейс «скрипта ещё
// нет вовсе». Первое подходящее правило побеждает:
//
//   1. Пусто       — last_seen_at пуст: скрипт ни разу не присылал данные.
//                    Новый ручной аккаунт без установленного скрипта; в ячейке
//                    «—», в группы и счётчики «По технике» не попадает (только
//                    в «Все»). Техника не судит, пока нет источника данных.
//   2. Бан         — last_seen_at протух (> watchdog_hours). Старые данные
//                    неактуальны — кабинет с тех пор замолчал.
//   3. Отклонены   — явный сигнал от скрипта, перебивает Модерацию (кампанию
//                    завернули, факт активности уже не важен).
//   4. По факту    — если хоть раз была активность (клик ИЛИ спенд > 0), отдаём
//                    то, что прислал скрипт: Крутит / Бюджет / Пауза-Оплата.
//   5. Модерация   — last_seen_at есть (значит скрипт уже работает), аккаунт
//                    младше moderation_hours от created_at, активности не
//                    было. Реклама на проверке у Google. Снимется первым же
//                    спендом/кликом (правило 4) либо по истечении окна.
//   6. По факту    — окно модерации закончилось, активности так и не было:
//                    отдаём reported (как правило «Пауза/Оплата»). Фикс бага
//                    «аккаунт навсегда залип в Модерации».
//
...
// маркируется, в Модерацию не залипнем).
//
// Разведение по last_seen_at:
//   пуст           → правило 1 («—», ни в одну тех-группу не попадает)
//   есть, свежий   → правила 3-6 (Отклонены / факт / Модерация / факт)
//   есть, протух   → правило 2 (Бан)
...
function effectiveTechStatus(a, watchdogHours, moderationHours) {
  if (!a) return ''
  // 1. Пусто — скрипт ни разу не присылал данные. Дальше не идём.
  if (!a.last_seen_at) return ''
  // 2. Бан — last_seen_at протух
  ...
  // 3. Отклонены — явный сигнал, перебивает Модерацию
  if (reported === 'Отклонены') return 'Отклонены'
  // 4. Была активность — доверяем тому, что прислал скрипт
  if (hadAnyActivity(a)) return reported
  // 5. Свежий аккаунт без активности — Модерация (окно от created_at)
  if (isModerationWindow(a, moderationHours)) return 'Модерация'
  // 6. Окно модерации закончилось, активности так и не было — reported
  return reported
}
  ...
  function cellContent(key, a, mt, marks) {
    switch (key) {
      ...
      case 'tech_status': {
        ...
        // то, что прислал скрипт. Пустой eff = скрипт ни разу не отвечал → «—»
        // даже если в БД лежит легаси-литерал (например, ручной импорт).
        // tech_note скрываем во всех вычисляемых/пустых случаях — он из
        ...
        const isComputed = isComputedBan || isComputedMod || isEmpty
        const info = isEmpty ? null : (techStatusInfo(eff) || techStatusInfo(a.tech_status))
        ...
        const tooltip = isEmpty
          ? 'Скрипт мониторинга ещё ни разу не присылал данные — установите его в Google Ads'
          : isComputedBan
        ...
        return (
          <span className="cell-sm" style={{color:info?info.c:'var(--t3)',fontFamily:'JetBrains Mono'}} title={tooltip}>
            {isEmpty ? '—' : (info?info.l:(a.tech_status||'—'))}
    ...
    <div>
      ...
      <table className="metric-table">
        ...
        <tbody>
          {viewDesc.map(m=>(
            <tr key={m.id}>
              <td title={m.metric_date} style={{whiteSpace:'nowrap'}}>{fmtShortDate(m.metric_date)}</td>
              <td>{m.clicks||'—'}</td>
              <td>{m.cpc_local||'—'}</td>
              <td>{m.cpc_usd||'—'}</td>
